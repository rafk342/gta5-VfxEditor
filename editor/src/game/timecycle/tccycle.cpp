...
{
	return GetFloat32_FromFloat16(m_keyframes[static_cast<int>(region)][timeSampleIndex].tc_vars[id]);
}

void tcCycle::SetKeyframeValue(Regions region, int id, int timeSampleIndex, float val)
{
	this->m_keyframes[static_cast<int>(region)][timeSampleIndex].tc_vars[id] = GetFloat16_FromFloat32(val);
}
