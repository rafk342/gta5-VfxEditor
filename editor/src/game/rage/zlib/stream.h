...
public:
	...
	bool Compress(pVoid data, u32 dataSize, pVoid& compressedBuffer, u32& compressedSize)
	{
		...
		if (status >= 0)
			LogInfo("zLibCompressor::Compress() -> Failed with status {}", status);
...
public:
	zLibDecompressor()
	{
		...
		if (status >= 0)
			LogInfo("zLibDecompressor() -> Init failed with status {}", status);
	}
	...
	bool Decompress(pVoid bufferOut, u32 bufferOutSize, pVoid bufferIn, u32 bufferInSize, u32& leftSize)
	{
		...
		if (status >= 0)
			LogInfo("zLibDecompressor::Decompress() -> Failed with status {}", status);
