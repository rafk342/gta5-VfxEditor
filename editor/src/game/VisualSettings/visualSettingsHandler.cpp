...
void VScontainer::updateContainer(VisualSettingsHandler* handler)
{
...
#if 0
    mlogger(std::format("p_Vsettings data size : {}", handler->p_Vsettings->data.GetSize()));
    ...
    for (auto& vec : paramsMap)
    {
        for (auto& item : vec.second)
        {
            ...
            {
                mlogger("========================================");
                mlogger(std::format("could not find {} hash = {} in \t {}", item.labelName, item.hash, vec.first));
                mlogger("========================================");
        }
    }
}
...
void VisualSettingsHandler::importData(std::string srcPath)
{
    ...
    if (fs::exists(newPath)) {  //aditional check
        fs::remove(newPath);
    }
    SetFuncN_Bytes();
}
...
//
    ...
    mlogger(std::format("export vSettings took : {}", t.GetElapsedSeconds()));
