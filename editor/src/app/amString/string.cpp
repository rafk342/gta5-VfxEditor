#include "app/amString/string.h"
#include "app/amString/stringwrapper.h"
#include <cwchar>
...
void String::ToWide(wchar_t* destination, int destinationSize, const char* source)
{
	...
	for (; source[i]; i++)
	{
		...
		assert(i < destinationSize && "String::ToWide() -> Buffer is too small!");
	}
	destination[i] = 0;
}
...
void String::ToAnsi(char* destination, int destinationSize, const wchar_t* source)
{
	...
	for (; source[i]; i++)
	{
		...
		assert(i < destinationSize && "String::ToAnsi() -> Buffer is too small!");
	}
	destination[i] = 0;
}
