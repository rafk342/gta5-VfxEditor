...
const char* amDllName = "TceApp.asi";
...
void ShutdownRageAm()
{
	...
	printf("calling !tceApp.dll::Shutdown\n");
...
int main()
{
	...
	std::string path = mPaths::getExecutableDir();
	path.append("\\");
	path.append(amDllName);

	amDllPath = path.c_str();
	std::cout << amDllPath << std::endl;
...
#if loader
	std::cout << "filesize : " << std::filesystem::file_size(path) << std::endl;
	InitRageAm();
#else
	ShutdownRageAm();
#endif 
